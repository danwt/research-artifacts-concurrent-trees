...
TEST_CASE("gen::boost::optional") {
  prop("equivalent to gen::maybe", [](const GenParams &params) {
    const auto gen = gen::arbitrary<int>();
    const auto maybeShrinkable = gen::maybe(gen)(params.random, params.size);
    const auto optionalShrinkable =
        shrinkable::map(gen::boost::optional(gen)(params.random, params.size),
  ...
  prop("chooses correct Arbitrary instance", [](const GenParams &params) {
...
TEST_CASE("showValue(boost::optional)") {
  prop("shows present value using rc::show", [](const Box &x) {
