...
TEST_CASE("seq::all") {
  prop("returns true if all elements match predicate", [](int value) {
    const auto n = *gen::inRange(0, 200);
    auto seq = seq::take(n, seq::repeat(value));
    RC_ASSERT(seq::all(seq, [=](int x) { return x == value; }));
  });

  prop("returns false if one element does not match predicate", [](int value) {
...
TEST_CASE("seq::at") {
  ...
  prop("returns Nothing for indexes past end", [](int x) {
