...
TEST_CASE("seq::drop") {
  ...
  prop("copies are equal", [](const std::vector<int> &elements) {
    std::size_t n = *gen::inRange<std::size_t>(0, (elements.size() + 1) * 2);
...
TEST_CASE("seq::dropWhile") {
  ...
  prop("copies are equal", [](const std::vector<int> &elements, int limit) {
    const auto pred = [=](int x) { return x < limit; };
...
TEST_CASE("seq::map") {
  ...
  prop("copies are equal", [](const std::vector<int> &elements, int x) {
    const auto mapper = [=](int a) { return a * x; };
    auto mapSeq = seq::map(seq::fromContainer(elements), mapper);
...
TEST_CASE("seq::zipWith") {
  prop("works with no sequences", [](int x) {
  ...
  prop("works with one sequence", [](const std::vector<int> &elements, int x) {
...
TEST_CASE("seq::join") {
  ...
  prop("returns a seq with the elements of all seqs joined together", [] {
    auto vectors = *gen;
    auto seqs =
        seq::map(seq::fromContainer(vectors), [](std::vector<int> &&vec) {
...
TEST_CASE("seq::mapcat") {
  prop("equivalent to seq::join(seq::map(...))", [](std::vector<int> elements) {
  ...
  prop("copies are equal", [](std::vector<int> elements) {
    const auto seq = seq::fromContainer(std::move(elements));
    const auto mapper = [](int a) { return seq::just(a, a + 1, a + 2); };
  ...
  SECTION("does not copy elements") {
    ...
    const auto mapper = [](Logger &&a) {
      return seq::just(std::move(a), Logger("extra"));
    };
...
TEST_CASE("seq::cast") {
  ...
  prop("copies are equal", [](const std::vector<uint8_t> &elements) {
