...
TEST_CASE("shrinkable::mapShrinks") {
  ...
  prop("leaves value unaffected", [](int x) {
...
TEST_CASE("shrinkable::filter") {
  prop("returns Nothing if predicate returns false for value", [](int x) {
...
TEST_CASE("shrinkable::mapcat") {
  SECTION("monad laws") {
    ...
    prop("right identity", [](Shrinkable<int> m) {
      const auto s =
          shrinkable::mapcat(m, [](int v) { return shrinkable::just(v); });
      RC_ASSERT(s == m);
    });

    prop("associativity", [](Shrinkable<int> m) {
  ...
  static const auto pairShrinkable = [](int a, int b) {
    return shrinkable::mapcat(countdownShrinkable(a), [=](int x) {
  ...
  prop("first tries to shrink first shrinkable", [] {
    ...
    const auto shrinks =
        seq::map(seq::take(xs.first, shrinkable::immediateShrinks(shrinkable)),
  ...
  prop("tries to shrink second shrinkable after first shrinkable", [] {
    ...
    const auto shrinks =
        seq::map(seq::drop(xs.first, shrinkable::immediateShrinks(shrinkable)),
...
TEST_CASE("shrinkable::postShrink") {
  ...
  prop("postShrink(s, fn::constant(Seq<T>())) == s", [](Shrinkable<int> s) {
  ...
  prop("tail of shrink sequence is always the appended shrinks", [] {
    ...
    const auto shrinkable = shrinkable::postShrink(
        ...
        [](const std::pair<int, int> &p) {
          return seq::map(countdownSeq(p.second),
                          [=](int x) { return std::make_pair(p.first, x); });
        });
    ...
    onAnyPath(shrinkable,
              [](const Shrinkable<std::pair<int, int>> &value,
                ...
                const auto tail =
                    seq::map(seq::fromIteratorRange(end(shrinkVec) - v.second,
                                                    end(shrinkVec)),
