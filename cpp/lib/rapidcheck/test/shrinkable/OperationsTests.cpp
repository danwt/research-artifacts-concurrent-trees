...
TEST_CASE("shrinkable::all") {
  prop("returns true if predicate returns true for all", [] {
    ...
    const auto shrinkable =
        shrinkable::shrinkRecur(x, [](int y) { return seq::range(y - 1, 0); });

    RC_ASSERT(shrinkable::all(
        shrinkable, [](const Shrinkable<int> &s) { return s.value() != 0; }));
  });
  ...
  prop("returns true if predicate returns false for at least one shrinkable",
       [] {
         ...
         RC_ASSERT(!shrinkable::all(shrinkable, [](const Shrinkable<int> &s) {
           return s.value() != 0;
         }));
       });
}
...
TEST_CASE("shrinkable::findLocalMin") {
  prop("returns the original value if no value matched predicate", [](int x) {
    auto result =
...
TEST_CASE("shrinkable::walkPath") {
  prop("returns the shrinkable at the end of the path", [] {
    ...
    const auto shrinkable = shrinkable::shrinkRecur(
        ...
        [](const std::vector<std::size_t> &lambdaPath) {
          return seq::map(seq::index(), [=](std::size_t i) {
            auto p = lambdaPath;
            p.push_back(i);
            return p;
          });
  ...
  prop("returns something if path never goes outside tree", [] {
    ...
    const auto shrinkable =
        shrinkable::shrinkRecur(std::size_t(0), [=](std::size_t x) {
          return seq::range<std::size_t>(0, limit);
        });
