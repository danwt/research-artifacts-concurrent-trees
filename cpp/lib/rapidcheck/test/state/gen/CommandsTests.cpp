...
TEST_CASE("state::gen::commands") {
  ...
  prop("passes the correct size", [](const GenParams &params) {
    const auto gen = state::gen::commands(IntVec(), &captureParams);
    onAnyPath(gen(params.random, params.size),
              [&](const Shrinkable<IntVecCmds> &value,
                  const Shrinkable<IntVecCmds> &shrink) {
                ...
                RC_ASSERT(std::all_of(
                    begin(allParams), end(allParams), [&](const GenParams &p) {
  ...
  prop("correctly threads the state when generating commands",
       [](const GenParams &params) {
         IntVec s0({0});
         const auto gen = state::gen::commands(s0, [](const IntVec &vec) {
  ...
  prop("finds minimum where one commands always fails",
       [](const GenParams &params, const IntVec &s0) {
         ...
         const auto result = searchGen(
             params.random, params.size, gen, [&](const IntVecCmds &cmds) {
               try {
                 IntVec sut = s0;
                 runAll(cmds, s0, sut);
               } catch (...) {
  ...
  prop(
      ...
      [](const GenParams &params) {
        ...
        RC_ASSERT(seq::any(
            shrinkable.shrinks(), [&](const Shrinkable<CommandsType> &shrink) {
      ...
      [](const GenParams &params) {
        ...
        RC_ASSERT(seq::any(
            shrinkable.shrinks(), [&](const Shrinkable<CommandsType> &shrink) {
              for (const auto &cmd : shrink.value()) {
                const auto tag =
                    static_cast<const TaggedCountdownCmd &>(*cmd).tag;
                if (tag == command.tag) {
                  return false;
                }
              }
  ...
  prop("works with non-copyable models", [](const GenParams &params) {
