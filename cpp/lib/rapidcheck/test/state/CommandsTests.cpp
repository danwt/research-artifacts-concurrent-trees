...
Gen<std::vector<IntVecCmdSP>> pushBackCommands() {
  return gen::container<std::vector<IntVecCmdSP>>(
      gen::exec([]() -> IntVecCmdSP { return std::make_shared<PushBack>(); }));
}
...
TEST_CASE("state::runAll") {
  prop("runs the commands in sequence", [](const IntVec &s0) {
    const auto cmds = *pushBackCommands();
...
TEST_CASE("state::isValidSequence") {
  prop("returns true if all commands are valid", [](IntVec s0) {
