...
TEST_CASE("state::check") {
  prop("if no command fails, check succeeds", [](const IntVec &s0, IntVec sut) {
    state::check(s0, sut, state::gen::execOneOfWithArgs<PushBack>());
  });

  prop("if some command fails, check fails", [](const IntVec &s0, IntVec sut) {
    try {
  ...
  prop("works with non-copyable models", [](const GenParams &params) {
    ...
    state::check(
        &initialNonCopyableModel, sut, [](const NonCopyableModel &model) {
          return state::gen::execOneOfWithArgs<NonCopyableInc,
                                               NonCopyableDec>()(model.value);
        });
  });
}
