...
struct ShrinkTowardsProperties {
  template <typename T>
  static void exec() {
    templatedProp<T>("first tries target immediately", [](T target) {
    ...
    templatedProp<T>("tries an adjacent value last", [](T target) {
      T value = *gen::distinctFrom(target);
      auto seq = shrink::towards(value, target);
    ...
    templatedProp<T>("never contains original value", [](T x, T y) {
...
struct IntegralProperties {
  ...
  static void exec() {
    ...
    templatedProp<T>("never contains original value", [](T x) {
  ...
  static void exec() {
    templatedProp<T>(
        "shrinks negative values to their positive equivalent", [] {
          T value = *gen::suchThat(gen::negative<T>(), [](T x) {
...
struct CharacterProperties {
  template <typename T>
  static void exec() {
    templatedProp<T>("'a' is the first shrink", [](T x) {
      RC_PRE(x != 'a');
    ...
    templatedProp<T>("never contains self", [](T x) {
