...
TEST_CASE("Random") {
  ...
  prop("different splits are inequal", [](Random r1) {
    Random r2(r1.split());
    RC_ASSERT(r1 != r2);
  });
  ...
  prop("different splits yield different sequences", [](Random r1) {
    Random r2(r1.split());
  ...
  prop("has uniform distribution", [] {
    ...
    double error = std::accumulate(
        begin(bins), end(bins), 0.0, [=](double lambdaError, std::uint64_t x) {
