...
TEST_CASE("gen::tuple") {
  ...
  prop("shrinks components from left to right", [] {
    ...
    const int x3 = *number;
    auto gen = gen::tuple(
        genFixedCountdown(x1), genFixedCountdown(x2), genFixedCountdown(x3));
    RC_ASSERT(shrinkable::all(
        gen(Random(), 0), [](const Shrinkable<std::tuple<int, int, int>> &s) {
  ...
  prop("works with non-copyable types", [](int v1, int v2) {
