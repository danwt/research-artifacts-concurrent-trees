...
TEST_CASE("gen::map") {
  ...
  prop("forwards the parameter to the generator", [](const GenParams &params) {
    const auto gen =
        gen::map(genPassedParams(), [](GenParams &&x) { return std::move(x); });
  ...
  prop(
      ...
      [](const Random &random) {
        ...
        expected[0] = '1';
        const auto result =
            searchGen(random, kNominalSize, gen, [=](const std::string &x) {
              return x.size() >= n;
            });
...
TEST_CASE("gen::mapcat") {
  ...
  prop("mapcats the returned shrinkables",
       [](Shrinkable<int> a, Shrinkable<int> b) {
         const auto expected = shrinkable::mapcat(a, [=](int x) {
           return shrinkable::map(b,
                                  [=](int y) { return std::make_pair(x, y); });
         });
         const auto gen = gen::mapcat<int>(
             fn::constant(a), [=](int x) -> Gen<std::pair<int, int>> {
  ...
  prop("passes correct size", [](const GenParams &params) {
    const auto gen = gen::mapcat(genSize(), [](int x) {
      return gen::map(genSize(), [=](int y) { return std::make_pair(x, y); });
    });
  ...
  prop("passes unique random generators", [](const GenParams &params) {
    const auto gen = gen::mapcat(genRandom(), [](const Random &x) {
      return gen::map(genRandom(), [=](Random &&y) {
  ...
  SECTION("works with non-copyable types") {
    const auto gen =
        gen::mapcat(gen::arbitrary<NonCopyable>(), [](NonCopyable &&x) {
...
TEST_CASE("gen::join") {
  ...
  prop("passes correct size", [](const GenParams &params) {
    const auto gen = gen::join(gen::map(genSize(), [=](int x) {
      return gen::map(genSize(), [=](int y) { return std::make_pair(x, y); });
    }));
  ...
  prop("passes unique random generators", [](const GenParams &params) {
    const auto gen = gen::join(gen::map(genRandom(), [=](const Random &x) {
      return gen::map(genRandom(),
                      [=](const Random &y) { return std::make_pair(x, y); });
    }));
  ...
  SECTION("works with non-copyable types") {
    const auto gen = gen::join(
        gen::map(gen::arbitrary<NonCopyable>(), [](const NonCopyable &x) {
          RC_ASSERT(isArbitraryPredictable(x));
          return gen::arbitrary<NonCopyable>();
        }));
...
TEST_CASE("gen::apply") {
  prop("has tuple shrinking semantics", [] {
    ...
    const auto gen =
        gen::apply([](int a, int b, int c) { return std::make_tuple(a, b, c); },
                   g1,
                   g2,
                   g3);
  ...
  prop("passes correct size", [](const GenParams &params) {
    ...
    RC_ASSERT(value == std::make_tuple(params.size, params.size, params.size));
  });

  prop("passed random generators are unique", [](const GenParams &params) {
    const auto gen = gen::apply(
...
TEST_CASE("gen::resize") {
  prop("always uses the specified size", [](const GenParams &params) {
...
TEST_CASE("gen::scale") {
  prop("scales the size by the specified factor", [](const GenParams &params) {
...
TEST_CASE("gen::noShrink") {
  ...
  prop("returned shrinkable has no shrinks", [](const GenParams &params) {
    const auto gen = gen::noShrink(gen::arbitrary<int>());
    const auto shrinkable = gen(params.random, params.size);
    RC_ASSERT(!shrinkable.shrinks().next());
  });

  prop("passes generation params unchanged", [](const GenParams &params) {
...
TEST_CASE("gen::withSize") {
  prop("passes the current size to the callable", [](const GenParams &params) {
    const auto gen = gen::withSize([](int size) { return gen::just(size); });
    const auto value = gen(params.random, params.size).value();
  ...
  prop("passes generation params unchanged", [](const GenParams &params) {
    const auto gen = gen::withSize([](int size) { return genPassedParams(); });
    const auto value = gen(params.random, params.size).value();
    RC_ASSERT(value == params);
  });
}
