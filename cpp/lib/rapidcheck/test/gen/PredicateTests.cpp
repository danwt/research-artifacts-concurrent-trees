...
TEST_CASE("gen::suchThat") {
  prop("generated value always matches predicate", [](const GenParams &params) {
...
TEST_CASE("gen::nonEmpty") {
  prop("never generates empty values", [](const GenParams &params) {
  ...
  prop("uses correct arbitrary instance", [](const GenParams &params) {
    const auto gen = gen::nonEmpty<std::vector<Predictable>>();
    const auto value = gen(params.random, params.size).value();
    RC_ASSERT(std::all_of(begin(value), end(value), [](const Predictable &p) {
      return isArbitraryPredictable(p);
    }));
  });
}
...
struct NonZeroProperties {
  template <typename T>
  static void exec() {
    templatedProp<T>("never generates zero", [=](const GenParams &params) {
