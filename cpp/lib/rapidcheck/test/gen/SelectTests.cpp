...
struct ElementOfContainerProperties {
  ...
  static void exec() {
    templatedProp<T>(
        "works with all basic container types", [](const GenParams &params) {
...
TEST_CASE("gen::elementOf") {
  ...
  prop("all elements are eventually generated", [](const GenParams &params) {
    const auto elements = *gen::nonEmpty<std::vector<int>>();
...
TEST_CASE("gen::weightedElement") {
  prop("respects weights", [](const GenParams &params) {
...
TEST_CASE("gen::sizedElementOf") {
  SECTION("when size is zero") {
    prop("generates only the first element", [](const Random &random) {
      const auto elements = *gen::nonEmpty<std::vector<int>>();
      const auto gen = gen::sizedElementOf(elements);
  ...
  SECTION("when size is kNominalSize") {
    prop("all elements are eventually generated", [](const Random &random) {
      const auto elements = *gen::nonEmpty<std::vector<int>>();
      tryUntilAll(std::set<int>(begin(elements), end(elements)),
  ...
  prop("all generated values are elements", [](const GenParams &params) {
    const auto elements = *gen::nonEmpty<std::vector<int>>();
    const auto gen = gen::sizedElementOf(elements);
    const auto value = gen(params.random, params.size).value();
    RC_ASSERT(std::find(begin(elements), end(elements), value) !=
  ...
  prop("first shrink is always the first element", [](const GenParams &params) {
    const auto elements = *gen::nonEmpty<std::vector<int>>();
    const auto gen = gen::sizedElementOf(elements);
    onAnyPath(gen(params.random, params.size),
              [&](const Shrinkable<int> &value, const Shrinkable<int> &shrink) {
                if (value.value() != elements.front()) {
                  RC_ASSERT(value.shrinks().next()->value() ==
                            elements.front());
                }
              });
  });
...
TEST_CASE("gen::oneOf") {
  ...
  prop("works with non-copyable types", [](const GenParams &params) {
...
TEST_CASE("gen::sizedOneOf") {
  ...
  prop("first shrink is using first generator", [](const GenParams &params) {
  ...
  prop(
      ...
      [](const GenParams &params) {
        ...
        const auto value = shrinkable.value();
        const auto expected =
            shrinkable::map(countdownShrinkable(value.first), [=](int x) {
              return std::make_pair(x, value.second);
            });
        const auto actual = shrinkable::mapShrinks(
            shrinkable, [=](Seq<Shrinkable<std::pair<int, int>>> &&shrinks) {
  ...
  prop(
      ...
      [](const GenParams &params) {
        ...
        const auto result = searchGen(
            params.random, params.size, gen, [=](const std::pair<int, int> &p) {
