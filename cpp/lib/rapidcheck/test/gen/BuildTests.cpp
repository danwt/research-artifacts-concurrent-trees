...
Gen<typename T::element_type> mapDeref(Gen<T> ptrGen) {
  return gen::map(std::move(ptrGen), [](T &&value) -> typename T::element_type {
...
TEST_CASE("gen::construct/gen::makeUnique/gen::makeShared") {
  prop("has tuple shrinking semantics", [] {
    ...
    const auto tupleShrinks = shrinkable::immediateShrinks(tupleShrinkable);
    ...
    const auto makeUniqueShrinkable = mapDeref(
        gen::makeUnique<std::tuple<int, int, int>>(g1, g2, g3))(Random(), 0);
    RC_ASSERT(makeUniqueShrinkable.value() == tupleValue);
    RC_ASSERT(shrinkable::immediateShrinks(makeUniqueShrinkable) ==
              tupleShrinks);

    const auto makeSharedShrinkable = mapDeref(
        gen::makeShared<std::tuple<int, int, int>>(g1, g2, g3))(Random(), 0);
...
TEST_CASE("gen::build") {
  prop("has tuple shrinking semantics", [] {
    const auto sizes = *gen::container<std::array<int, 5>>(gen::inRange(0, 10));
    ...
    const auto gen =
        gen::build(gen::construct<Foobar<int>>(gens[0]),
  ...
  prop("passes unique random generators", [](const GenParams &params) {
    const auto gen = gen::build(
        gen::construct<Foobar<Random>>(genRandom()),
        gen::set(&Foobar<Random>::setB, genRandom()),
        gen::set(&Foobar<Random>::setCD, gen::tuple(genRandom(), genRandom())),
  ...
  SECTION("uses correct arbitrary instances when generator not specified") {
    const auto gen =
        gen::build(gen::construct<Foobar<Predictable>, Predictable>(),
