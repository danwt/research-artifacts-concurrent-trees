...
TEST_CASE("shrinkRecipe") {
  prop("Random and size remain the same", [](const Recipe &recipe) {
    RC_ASSERT(seq::all(shrinkRecipe(recipe), [&](const Recipe &shrink) {
      return (shrink.random == recipe.random) && (shrink.size == recipe.size);
    }));
  });

  prop("does not shrink fixed values", [](const Recipe &recipe) {
    RC_ASSERT(seq::all(shrinkRecipe(recipe), [&](const Recipe &shrink) {
  ...
  prop("does not change any values except last one", [](const Recipe &recipe) {
    RC_ASSERT(seq::all(shrinkRecipe(recipe), [&](const Recipe &shrink) {
  ...
  prop("sets all values except last to fixed", [](const Recipe &recipe) {
    RC_ASSERT(seq::all(shrinkRecipe(recipe), [&](const Recipe &shrink) {
  ...
  prop("last value is a shrink of the original value",
       [](const Recipe &recipe) {
         RC_ASSERT(seq::all(shrinkRecipe(recipe), [&](const Recipe &shrink) {
           ...
           return seq::any(std::move(shrinks), [&](const Shrinkable<Any> &s) {
             return equalAsInt(s, shrink.ingredients[i].shrinkable);
           });
         }));
       });

  prop("maintains description of shrunk ingredient", [](const Recipe &recipe) {
    RC_ASSERT(seq::all(shrinkRecipe(recipe), [&](const Recipe &shrink) {
      const auto i = shrink.ingredients.size() - 1;
  ...
  prop("yields empty sequence if all values are fixed", [](Recipe recipe) {
  ...
  prop(
      ...
      [] {
        ...
        const auto recipe = *gen::suchThat<Recipe>(
