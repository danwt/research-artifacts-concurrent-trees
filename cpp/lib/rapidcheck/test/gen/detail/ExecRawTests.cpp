...
TEST_CASE("execRaw") {
  SECTION("uses correct arbitrary instance for arguments") {
    const auto values = execRaw([](const Predictable &a,
                                   const Predictable &b,
                                   const Predictable &c) {
  ...
  SECTION("shrinks arguments like a tuple") {
    ...
    const auto mappedShrinkable =
        shrinkable::map(execShrinkable, [](std::pair<TupleT, Recipe> &&x) {
  ...
  prop("passes on the correct size", [] {
    ...
    auto valueShrinkable = shrinkable::map(
        shrinkable, [](std::pair<std::vector<int>, Recipe> &&x) {
          return std::move(x.first);
        });

    RC_ASSERT(shrinkable::all(
        valueShrinkable, [=](const Shrinkable<std::vector<int>> &x) {
          auto sizes = x.value();
          return std::all_of(begin(sizes), end(sizes), [=](int sz) {
            return sz == expectedSize;
          });
        }));
  });

  prop("passed generators are unique", [](const Random &initial) {
    const auto n = *gen::inRange<std::size_t>(1, 10);
  ...
  prop("passed randoms do not change with shrinking",
       [](const Random &initial) {
         ...
         const auto valueShrinkable = shrinkable::map(
             shrinkable, [](std::pair<std::vector<Random>, Recipe> &&x) {
               return std::move(x.first);
             });

         const auto head = valueShrinkable.value();
         RC_ASSERT(shrinkable::all(
             valueShrinkable, [=](const Shrinkable<std::vector<Random>> &x) {
               return x.value() == head;
             }));
       });
  ...
  prop("disallows nested use of operator*", [](const GenParams &params) {
