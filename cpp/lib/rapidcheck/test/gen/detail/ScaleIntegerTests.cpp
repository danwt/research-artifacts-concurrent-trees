...
TEST_CASE("scaleInteger") {
  prop("for uint32_t, equal to naive way", [] {
    ...
    RC_ASSERT(
        gen::detail::scaleInteger(x, size) ==
        ((x * std::min<uint64_t>(kNominalSize, size) + (kNominalSize / 2)) /
         kNominalSize));
  });

  prop("result strictly increases with size", [](uint64_t x) {
  ...
  prop("result strictly increases with value", [](uint64_t a, uint64_t b) {
    const auto size = *gen::nonNegative<int>();
  ...
  prop("yields input for kNominalSize", [](uint64_t x) {
