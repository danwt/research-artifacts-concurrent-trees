...
TEST_CASE("gen::character") {
  prop("never generates null characters", [](const GenParams &params) {
  ...
  prop("first shrink is always 'a')", [](const GenParams &params) {
    const auto gen = gen::character<char>();
  ...
  static void exec() {
    ...
    templatedProp<T>(
        ...
        [](const Random &random) {
          ...
          const auto result =
              searchGen(random, size, gen::string<T>(), [=](const T &x) {
                return x.size() >= n;
              });
          T expected(n, 'a');
    ...
    templatedProp<T>("first shrink is empty", [](const GenParams &params) {
      const auto gen = gen::string<T>();
      const auto shrinkable = gen(params.random, params.size);
