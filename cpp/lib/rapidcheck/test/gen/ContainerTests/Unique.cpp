...
struct UniqueByFactory {
  ...
  static Gen<Container> makeGen(std::size_t count, Gen<std::pair<T1, T2>> gen) {
    return gen::uniqueBy<Container>(
        count, std::move(gen), [](const std::pair<T1, T2> &p) {
          return p.first;
        });
  }
};
...
struct UniqueByProperties {
  ...
  static void exec() {
    templatedProp<T>(
        "generated values are unique by key", [](const GenParams &params) {
    ...
    templatedProp<T>(
        ...
        [](const GenParams &params) {
          ...
          const auto result =
              searchGen(params.random, params.size, gen, [=](T elements) {
    ...
    templatedProp<T>(
        ...
        [](const GenParams &params) {
          ...
          const auto gen = Factory::template makeGen<T>(
              count, gen::arbitrary<std::pair<int, int>>());
          onAnyPath(
              gen(params.random, params.size),
              [=](const Shrinkable<T> &value, const Shrinkable<T> &shrink) {
                const auto v = value.value();
...
struct UniqueProperties {
  ...
  static void exec() {
    templatedProp<T>(
        "generated values are unique", [](const GenParams &params) {
    ...
    templatedProp<T>(
        ...
        [](const GenParams &params) {
          const auto count = *gen::inRange<std::size_t>(0, 10);
          const auto gen =
              Factory::template makeGen<T>(count, gen::arbitrary<int>());
          onAnyPath(
              gen(params.random, params.size),
              [=](const Shrinkable<T> &value, const Shrinkable<T> &shrink) {
                const auto v = value.value();
