...
struct IntegralProperties {
  ...
  static void exec() {
    TEMPLATED_SECTION(T, "when size >= gen::kNominalSize") {
      templatedProp<T>("all bits can be either 1 or 0", [](Random random) {
        T ones = 0;
        T zeroes = 0;
...
struct NumericProperties {
  template <typename T>
  static void exec() {
    templatedProp<T>("zero size always yields zero", [](const Random &random) {
...
struct SignedProperties {
  template <typename T>
  static void exec() {
    templatedProp<T>("P(value > 0) ~ P(value < 0)", [](Random random) {
...
struct InRangeProperties {
  ...
  static void exec() {
    ...
    templatedProp<T>(
        "never generates values outside of range", [](const GenParams &params) {
    ...
    templatedProp<T>("throws if min <= max", [](const GenParams &params) {
      const auto a = *gen::arbitrary<T>();
      const auto b = *gen::distinctFrom(a);
    ...
    templatedProp<T>("first shrink is min", [](const GenParams &params) {
      // TODO range generator
      const auto range = *genRange;
      const auto shrinkable = gen::inRange<T>(range.first, range.second)(
          params.random, params.size);
    ...
    templatedProp<T>(
        "when size == 0, generates only min", [](const Random &random) {
          const auto range = *genRange;
          RC_ASSERT(gen::inRange(range.first, range.second)(random, 0) ==
