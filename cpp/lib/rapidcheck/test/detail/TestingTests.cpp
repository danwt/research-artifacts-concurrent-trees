...
TEST_CASE("searchProperty") {
  ...
  prop("runs all test cases if no cases fail", [](const TestParams &params) {
    auto numCases = 0;
  ...
  prop("does not include empty tags in tags", [](const TestParams &params) {
...
Shrinkable<CaseDescription> countdownEven(int start) {
  return shrinkable::map(countdownShrinkable(start), [=](int x) {
    CaseDescription desc;
    desc.result.type =
        ((x % 2) == 0) ? CaseResult::Type::Failure : CaseResult::Type::Success;
...
TEST_CASE("shrinkTestCase") {
  prop("returns the minimum shrinkable", [] {
    ...
    const auto shrinkable = shrinkable::map(
        shrinkable::shrinkRecur(std::numeric_limits<int>::max(),
                                [](int x) { return shrink::towards(x, 0); }),
        [=](int x) {
          CaseDescription desc;
          desc.result.type = (x >= target) ? CaseResult::Type::Failure
    ...
    RC_ASSERT(result.first.value().result.type == CaseResult::Type::Failure);
  ...
  prop("calls onShrinkTried for each shrink tried", [] {
    ...
    listener.onShrinkTriedCallback = [&](const CaseDescription &desc,
                                         bool accepted) {
...
TEST_CASE("testProperty") {
  prop("returns the correct shrink path on a failing case",
       [](TestParams params) {
         ...
         const auto evenInteger =
             gen::scale(0.25, gen::suchThat(gen::positive<int>(), [](int x) {
                          return (x % 2) == 0;
                        }));
  ...
  prop("on failure, description contains message",
       [](const TestParams &params, const std::string &description) {
         RC_PRE(params.maxSuccess > 0);
         const auto results =
             testTestable([&] { RC_FAIL(description); }, params, dummyListener);
  ...
  prop("correctly reports test case distribution", [] {
    auto allTags = *gen::container<std::vector<std::vector<std::string>>>(
