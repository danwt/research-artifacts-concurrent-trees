...
TEST_CASE("CaseDescription") {
  SECTION("operator==/operator!=") {
    ...
    prop("not equal if example not equal", [](const CaseDescription &original) {
      auto other(original);
...
TEST_CASE("PropertyAdapter") {
  prop("Discard overrides any other reported result", [] {
    auto results = *gen::container<std::vector<CaseResult>>(genResultOfType(
        {CaseResult::Type::Success, CaseResult::Type::Failure}));
    const auto discardResult = *genResultOfType({CaseResult::Type::Discard});
  ...
  prop("Failure overrides any reported successes", [] {
    ...
    const auto failureResult = *genResultOfType({CaseResult::Type::Failure});
    const auto position = *gen::inRange<std::size_t>(0, results.size());
  ...
  prop("result description contains descriptions of all reported failures", [] {
    auto failureResults = *gen::container<std::vector<CaseResult>>(
  ...
  prop("returns CaseResult as is", [](const CaseResult &result) {
...
TEST_CASE("toProperty") {
        ...
        const auto gen = toProperty([=] {
          for (std::size_t i = 0; i < n; i++) {
            if (i == throwIndex) {
              ...
              try {
                // Introduce a dummy variable to prevent double-free error on
                // LLVM 8.0.0.
                auto dummy =
  ...
  prop("tags correspond to counterexample", [](const GenParams &params) {
    const auto gen = toProperty([=] {
      const auto tags =
