...
TEST_CASE("BitStream") {
  SECTION("next") {
    ...
    prop("spills no bits", [=](uint64_t x) {
      auto source = makeSource(seq::repeat(x));
      auto stream = bitStreamOf(source);

      const auto sizes =
          *gen::suchThat(bitSizes, [](const std::vector<int> &y) {
            return std::accumulate(begin(y), end(y), 0) >= 64;
          });
    ...
    prop("does not return more bits than requested (signed)", [=](uint64_t x) {
      auto source = makeSource(seq::repeat(x));
      auto stream = bitStreamOf(source);
    ...
    prop("does not return any bits when none are requested", [=](uint64_t x) {
      auto source = makeSource(seq::repeat(x));
      auto stream = bitStreamOf(source);
    ...
    prop("works with booleans", [](uint64_t x) {
