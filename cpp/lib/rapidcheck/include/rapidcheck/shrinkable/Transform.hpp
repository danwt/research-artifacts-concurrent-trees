...
Maybe<Shrinkable<T>> filter(Shrinkable<T> shrinkable, Predicate &&pred) {
  ...
  return shrinkable::mapShrinks(
      std::move(shrinkable), [=](Seq<Shrinkable<T>> &&shrinks) {
        return seq::mapMaybe(std::move(shrinks), [=](Shrinkable<T> &&shrink) {
...
Shrinkable<T> postShrink(Shrinkable<T> shrinkable, Shrink &&shrink) {
  return shrinkable::mapcat(std::move(shrinkable), [=](T &&x) {
    return shrinkable::shrinkRecur(std::move(x), shrink);
  });
}
