...
public:
  ...
  Shrinkables<T> generateElements(const Random &random,
    ...
    return generateShrinkables(
        random, size, count, gen, [&](const Shrinkable<T> &s) {
  ...
  Seq<Shrinkables<T>> shrinkElements(const Shrinkables<T> &shrinkables) const {
    ...
    return shrink::eachElement(shrinkables, [=](const Shrinkable<T> &s) {
      return seq::filter(s.shrinks(), [=](const Shrinkable<T> &x) {
...
public:
  ...
  ShrinkablePairs<K, V> generateElements(const Random &random,
    ...
    auto keyShrinkables = generateShrinkables(
        r.split(), size, count, keyGen, [&](const Shrinkable<K> &s) {
...
public:
  ...
  Shrinkables<T> generateElements(const Random &random,
    ...
    std::set<Key> values;
    return detail::generateShrinkables(
        random, size, count, gen, [&](const Shrinkable<T> &s) {
          return values.insert(m_f(s.value())).second;
        });
  }
  ...
  Seq<Shrinkables<T>> shrinkElements(const Shrinkables<T> &shrinkables) const {
    ...
    return shrink::eachElement(shrinkables, [=](const Shrinkable<T> &s) {
      ...
      return seq::filter(s.shrinks(), [=](const Shrinkable<T> &shrink) {
        const auto shrinkKey = m_f(shrink.value());
        return (!(valueKey < shrinkKey) && !(shrinkKey < valueKey)) ||
...
struct DefaultArbitrary<std::vector<T, Allocator>> {
  static Gen<std::vector<T, Allocator>> arbitrary() {
    return gen::container<std::vector<T, Allocator>>(gen::arbitrary<T>());
  }
};
...
struct DefaultArbitrary<std::deque<T, Allocator>> {
  static Gen<std::deque<T, Allocator>> arbitrary() {
    return gen::container<std::deque<T, Allocator>>(gen::arbitrary<T>());
  }
};
...
struct DefaultArbitrary<std::forward_list<T, Allocator>> {
  static Gen<std::forward_list<T, Allocator>> arbitrary() {
    return gen::container<std::forward_list<T, Allocator>>(gen::arbitrary<T>());
  }
};
...
struct DefaultArbitrary<std::list<T, Allocator>> {
  static Gen<std::list<T, Allocator>> arbitrary() {
    return gen::container<std::list<T, Allocator>>(gen::arbitrary<T>());
  }
};
...
struct DefaultArbitrary<
    std::unordered_multiset<Key, Hash, KeyEqual, Allocator>> {
  static Gen<std::unordered_multiset<Key, Hash, KeyEqual, Allocator>>
  arbitrary() {
...
struct DefaultArbitrary<std::unordered_map<Key, T, Hash, KeyEqual, Allocator>> {
  static Gen<std::unordered_map<Key, T, Hash, KeyEqual, Allocator>>
  arbitrary() {
    return gen::container<
...
struct DefaultArbitrary<
    std::unordered_multimap<Key, T, Hash, KeyEqual, Allocator>> {
  static Gen<std::unordered_multimap<Key, T, Hash, KeyEqual, Allocator>>
  arbitrary() {
    return gen::container<
        std::unordered_multimap<Key, T, Hash, KeyEqual, Allocator>>(
        gen::arbitrary<Key>(), gen::arbitrary<T>());
...
Gen<Container> unique(std::size_t count, Gen<T> gen) {
  return gen::uniqueBy<Container>(
      count, std::move(gen), [](const T &x) -> const T & { return x; });
}
