...
public class LogicalOrderingAVL<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
     ...
     */
    final private V insert(final K key, final V item, boolean putIfAbsent, boolean isReplace,
                           Object replaceItem) {
        final Comparable<? super K> value = comparable(key);
            ...
            if (pred.valid) {
                ...
                if (predRes > 0) {
                    ...
                    if (res2 <= 0) {
                        ...
                        final AVLMapNode<K, V> newNode = new AVLMapNode<K, V>(key, item, pred,
                                succ, parent);
                        succ.pred = newNode;
                        pred.succ = newNode;
                        pred.unlockSuccLock();
    ...
    final private AVLMapNode<K, V> chooseParent(final AVLMapNode<K, V> pred,
                                                final AVLMapNode<K, V> succ, final AVLMapNode<K,
            V> firstCand) {
     ...
     */
    final private void insertToTree(final AVLMapNode<K, V> parent, final AVLMapNode<K, V> newNode,
    ...
    final private AVLMapNode<K, V> acquireTreeLocks(final AVLMapNode<K, V> node) {
        while (true) {
            ...
            final AVLMapNode<K, V> succRightChild = successor.right; // there is no left child to
            // the successor, perhaps there is a right one, which we need to lock.
     ...
     */
    final private boolean updateHeight(AVLMapNode<K, V> child, AVLMapNode<K, V> node,
                                       boolean isLeft) {
     ...
     */
    final private void rotate(final AVLMapNode<K, V> child, final AVLMapNode<K, V> node,
                              final AVLMapNode<K, V> parent, boolean left) {
        if (parent.left == node) {
            parent.left = child;
        } else {
            parent.right = child;
            ...
            public Iterator<java.util.Map.Entry<K, V>> iterator() {
                return new Iterator<Map.Entry<K, V>>() {
                    ...
                    public java.util.Map.Entry<K, V> next() {
                        getNext();
                        curr = currNext;
                        return curr == root ? null : new SimpleImmutableEntry<K, V>(curr.key,
                                (V) curr.item);
                    }
    ...
    class AVLMapNode<K, V> {
        ...
        /**
         * The lock that protects the node's tree fields, that is, {@code parent, left, right,
         ...
         */
        public AVLMapNode(final K key, final Object item, final AVLMapNode<K, V> pred,
                          final AVLMapNode<K, V> succ, final AVLMapNode<K, V> parent) {
            this.key = key;
            this.item = item;
            valid = true;
    ...
    boolean trueBalance() {
        ...
        Recursive<Function<AVLMapNode<K, V>, Integer>> f = new Recursive<>();
    ...
    boolean noLockHeld(){
        ...
        Recursive<Consumer<AVLMapNode<K, V>>> f = new Recursive<>();
