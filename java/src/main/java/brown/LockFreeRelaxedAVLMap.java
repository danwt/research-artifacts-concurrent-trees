...
public class LockFreeRelaxedAVLMap<K, V> {
    private final int d; // number of violations to allow on a search path before we fix
    // everything on it
    private static final int DEFAULT_d = 15; // experimentally determined to yield good
    // performance for both random workloads, and operations on sorted sequences
    private final Node root;
    private final Operation dummy;
    private final Comparator<? super K> comparator;
    private final AtomicReferenceFieldUpdater<LockFreeRelaxedAVLMap.Node,
            LockFreeRelaxedAVLMap.Node> updateLeft, updateRight;
    private final AtomicReferenceFieldUpdater<LockFreeRelaxedAVLMap.Node,
            LockFreeRelaxedAVLMap.Operation> updateOp;
    ...
    public LockFreeRelaxedAVLMap(final int allowedViolationsPerPath,
        ...
        dummy = new Operation();
        root = new Node(null, null, 0, 0, new Node(null, null, 0, 0, null, null, dummy), null,
                dummy);
    }
    ...
    private V doPut(final K key, final V value, final boolean onlyIfAbsent) {
        ...
        while (true) {
            ...
            if (helpSCX(op, 0)) {
                ...
                // we may have found the key and replaced its value (the old value is stored in
                // the old node)
    ...
    public final V remove(final K key) {
        ...
        while (true) {
            ...
            if (helpSCX(op, 0)) {
                // clean up violations if necessary
                if (d == 0) {
                    if ((p.tag >= 0 || ((l == p.left) ? r(p) : l(p)) == 1) && !isSentinel(nodes[1]))
                        fixToKey(k);
                } else {
                    if (count >= d) fixToKey(k);
                }
    ...
    private boolean helpSCX(final Operation op, int i) {
        // get local references to some fields of op, in case we later null out fields of op (to
        ...
        // freeze sub-tree
        for (; i < ops.length; ++i) {
            if (!updateOp.compareAndSet(nodes[i], ops[i], op) && nodes[i].op != op) { // if work
                // was not done
                if (op.allFrozen) {
                    return true;
                } else {
        ...
        if (nodes[0].left == nodes[1]) {
            updateLeft.compareAndSet(nodes[0], nodes[1], subtree);     // splice in new sub-tree
            // (as a left child)
        } else { // assert: nodes[0].right == nodes[1]
            updateRight.compareAndSet(nodes[0], nodes[1], subtree);    // splice in new sub-tree
            // (as a right child)
        }
    ...
    private Operation createInsertOp(final Node p, final Node l, final K key, final V value,
                                     Comparable k) {
        ...
        final int newTag = (isSentinel(l) ? 0 : l.tag - 1);               // (maintain sentinel
        // tags at 0)
    ...
    private Operation createDeleteOp(final Node gp, final Node p, final Node l) {
        ...
        final int newTag = (isSentinel(p) ? 0 : p.tag + s.tag + 1 + (left ? l(p) : r(p))); //
        // tags of parent + sibling of deleted leaf, plus fudge factor
    ...
    private Operation createBalancingOp(final Node f, final Node fX, final Node fXX) {
        ...
        } else { // assert(fXX.tag > 0); // otherwise fXX isn't a violation, and fixToKey
            // wouldn't have invoked this.
            final Operation opfXL = weakLLX(fXL);
            if (opfXL == null) {
                return null;
            }
    ...
    private Operation createNegativeLeftOp(final Node f, final Node fX, final Node fXL,
                                           final Operation opf, final Operation opfX,
                                           final Operation opfXL) {
        if (fX.balance <= 0) {
            ...
            if (fXL.balance >= 0) {
                ...
                if (fXLR.tag == -1) {
                    return createPul8Op(new Node[]{f, fX, fXL, fXLR}, new Operation[]{opf, opfX,
                            opfXL, opfXLR});
                } else if (fXLR.tag == 0) {
                    return createPul7Op(new Node[]{f, fX, fXL, fXLR}, new Operation[]{opf, opfX,
                            opfXL, opfXLR});
                } else {
                    return createPul6Op(new Node[]{f, fX, fXL, fXLR}, new Operation[]{opf, opfX,
                            opfXL, opfXLR});
                }
            }
        }
    ...
    private Operation createPositiveLeftOp(final Node f, final Node fX, final Node fXL,
                                           final Node fXR,
                                           final Operation opf, final Operation opfX,
                                           final Operation opfXL, final Operation opfXR) {
        if (fX.balance >= 0) {
            return createPushOp(new Node[]{f, fX, fXL}, new Operation[]{opf, opfX, opfXL});
        } else { // assert(fX.balance == -1);
            if (fXR.tag > 0) {
                return createPsh9Op(new Node[]{f, fX, fXL, fXR}, new Operation[]{opf, opfX, opfXL
                        , opfXR});
            } else { // assert(fXR.tag == 0);
                if (fXR.balance <= 0) {
                    return createPsh10Op(new Node[]{f, fX, fXL, fXR}, new Operation[]{opf, opfX,
                            opfXL, opfXR});
                    ...
                    if (fXRL.tag == -1) {
                        return createPsh13Op(new Node[]{f, fX, fXL, fXR, fXRL},
                                new Operation[]{opf, opfX, opfXL, opfXR, opfXRL});
                    } else if (fXRL.tag == 0) {
                        return createPsh12Op(new Node[]{f, fX, fXL, fXR, fXRL},
                                new Operation[]{opf, opfX, opfXL, opfXR, opfXRL});
                    } else {
                        return createPsh11Op(new Node[]{f, fX, fXL, fXR, fXRL},
                                new Operation[]{opf, opfX, opfXL, opfXR, opfXRL});
                    }
                }
            }
    ...
    public static final class Node {
        ...
        public Node(final Object key, final Object value, final int tag, final int balance,
                    final Node left, final Node right, final Operation op) {
            this.key = key;
            this.value = value;
    ...
    public static final class Operation {
        ...
        public Operation() {            // create an dummy operation [[ we have null fields
            // rather than smaller records simply to avoid the overhead of inheritance ]]
            nodes = null;
            ops = null;
            subtree = null;
            this.state = STATE_ABORTED; // cheap trick to piggy-back on a pre-existing check for
            // frozen nodes
        }
    ...
    private Operation createNegativeRightOp(final Node f, final Node fX, final Node fXR,
        ...
        if (fX.balance >= 0) {
            ...
            if (fXR.balance <= 0) {
                ...
                if (fXRL.tag == -1) {
                    return createPul8SymOp(new Node[]{f, fX, fXR, fXRL}, new Operation[]{opf,
                            opfX, opfXR, opfXRL});
                } else if (fXRL.tag == 0) {
                    return createPul7SymOp(new Node[]{f, fX, fXR, fXRL}, new Operation[]{opf,
                            opfX, opfXR, opfXRL});
                } else {
                    return createPul6SymOp(new Node[]{f, fX, fXR, fXRL}, new Operation[]{opf,
                            opfX, opfXR, opfXRL});
                }
            }
        }
    ...
    private Operation createPositiveRightOp(final Node f, final Node fX, final Node fXR,
                                            final Node fXL,
                                            final Operation opf, final Operation opfX,
                                            final Operation opfXR, final Operation opfXL) {
        if (fX.balance <= 0) {
            return createPushSymOp(new Node[]{f, fX, fXR}, new Operation[]{opf, opfX, opfXR});
        } else { // assert(fX.balance == -1);
            if (fXL.tag > 0) {
                return createPsh9SymOp(new Node[]{f, fX, fXL, fXR}, new Operation[]{opf, opfX,
                        opfXL, opfXR});
            } else { // assert(fXL.tag == 0);
                if (fXL.balance >= 0) {
                    return createPsh10SymOp(new Node[]{f, fX, fXL, fXR}, new Operation[]{opf,
                            opfX, opfXL, opfXR});
                } else { // assert(fXL.balance == -1);
                    final Node fXLR = fXL.right;
                    final Operation opfXLR = weakLLX(fXLR);
                    ...
                    if (fXLR.tag == -1) {
                        return createPsh13SymOp(new Node[]{f, fX, fXL, fXR, fXLR},
                                new Operation[]{opf, opfX, opfXL, opfXR, opfXLR});
                    } else if (fXLR.tag == 0) {
                        return createPsh12SymOp(new Node[]{f, fX, fXL, fXR, fXLR},
                                new Operation[]{opf, opfX, opfXL, opfXR, opfXLR});
                    } else {
                        return createPsh11SymOp(new Node[]{f, fX, fXL, fXR, fXLR},
                                new Operation[]{opf, opfX, opfXL, opfXR, opfXLR});
                    }
                }
            }
    ...
    private Operation createPullOp(final Node[] nodes, final Operation[] ops) {
        final Node nodeXL = new Node(nodes[2].key, nodes[2].value, 0, nodes[2].balance,
                nodes[2].left, nodes[2].right, dummy);
        final int tag = (isSentinel(nodes[1]) ? 0 : nodes[1].tag - 1 + r(nodes[1])); // root of
        // old subtree is a sentinel
        final Node nodeX = new Node(nodes[1].key, nodes[1].value, tag, nodes[1].balance + 1,
                nodeXL, nodes[1].right, dummy);
        return new Operation(nodes, ops, nodeX);
    }

    private Operation createPushOp(final Node[] nodes, final Operation[] ops) {
        final Node nodeXL = new Node(nodes[2].key, nodes[2].value, nodes[2].tag - 1,
                nodes[2].balance, nodes[2].left, nodes[2].right, dummy);
        final int tag = (isSentinel(nodes[1]) ? 0 : nodes[1].tag + l(nodes[1])); // root of old
        // subtree is a sentinel
        final Node nodeX = new Node(nodes[1].key, nodes[1].value, tag, nodes[1].balance - 1,
                nodeXL, nodes[1].right, dummy);
        return new Operation(nodes, ops, nodeX);
    }
    ...
    private Operation createPul5Op(final Node[] nodes, final Operation[] ops) {
        final Node nodeXR = new Node(nodes[1].key, nodes[1].value, 0, 1 - l(nodes[2]),
                nodes[2].right, nodes[1].right, dummy);
        final int tag = (isSentinel(nodes[1]) ? 0 : nodes[1].tag - 1 + l(nodes[2])); // root of
        // old subtree is a sentinel
        final Node nodeX = new Node(nodes[2].key, nodes[2].value, tag, nodes[2].balance - 1,
                nodes[2].left, nodeXR, dummy);
        return new Operation(nodes, ops, nodeX);
    }

    private Operation createPul6Op(final Node[] nodes, final Operation[] ops) {
        final Node nodeXLR = new Node(nodes[3].key, nodes[3].value, nodes[3].tag - 1,
                nodes[3].balance, nodes[3].left, nodes[3].right, dummy);
        final Node nodeXL = new Node(nodes[2].key, nodes[2].value, 0, 0, nodes[2].left, nodeXLR,
                dummy);
        final int tag = nodes[1].tag;
        final Node nodeX = new Node(nodes[1].key, nodes[1].value, tag, 1, nodeXL, nodes[1].right,
                dummy);
        return new Operation(nodes, ops, nodeX);
    }

    private Operation createPul7Op(final Node[] nodes, final Operation[] ops) {
        final Node nodeXL = new Node(nodes[2].key, nodes[2].value, 0, r(nodes[3]), nodes[2].left,
                nodes[3].left, dummy);
        final Node nodeXR = new Node(nodes[1].key, nodes[1].value, 0, -l(nodes[3]),
                nodes[3].right, nodes[1].right, dummy);
        final int tag = nodes[1].tag;
    ...
    private Operation createPul8Op(final Node[] nodes, final Operation[] ops) {
        final Node nodeXL = new Node(nodes[2].key, nodes[2].value, 0, r(nodes[3]) - 1,
                nodes[2].left, nodes[3].left, dummy);
        final Node nodeXR = new Node(nodes[1].key, nodes[1].value, 0, 1 - l(nodes[3]),
                nodes[3].right, nodes[1].right, dummy);
        final int tag = (isSentinel(nodes[1]) ? 0 : nodes[1].tag - 1); // root of old subtree is
        // a sentinel
        final Node nodeX = new Node(nodes[3].key, nodes[3].value, tag, nodes[3].balance, nodeXL,
                nodeXR, dummy);
        return new Operation(nodes, ops, nodeX);
    }
    ...
    private Operation createPsh9Op(final Node[] nodes, final Operation[] ops) {
        final Node nodeXL = new Node(nodes[2].key, nodes[2].value, nodes[2].tag - 1,
                nodes[2].balance, nodes[2].left, nodes[2].right, dummy);
        final Node nodeXR = new Node(nodes[3].key, nodes[3].value, nodes[3].tag - 1,
                nodes[3].balance, nodes[3].left, nodes[3].right, dummy);
        final int tag = (isSentinel(nodes[1]) ? 0 : nodes[1].tag + 1); // root of old subtree is
        // a sentinel
    ...
    private Operation createPsh10Op(final Node[] nodes, final Operation[] ops) {
        final Node nodeXLL = new Node(nodes[2].key, nodes[2].value, nodes[2].tag - 1,
                nodes[2].balance, nodes[2].left, nodes[2].right, dummy);
        final Node nodeXL = new Node(nodes[1].key, nodes[1].value, 0, r(nodes[3]) - 1, nodeXLL,
                nodes[3].left, dummy);
        final int tag = (isSentinel(nodes[1]) ? 0 : nodes[1].tag + r(nodes[3])); // root of old
        // subtree is a sentinel
        final Node nodeX = new Node(nodes[3].key, nodes[3].value, tag, nodes[3].balance + 1,
                nodeXL, nodes[3].right, dummy);
        return new Operation(nodes, ops, nodeX);
    }
    ...
    private Operation createPsh11Op(final Node[] nodes, final Operation[] ops) {
        final Node nodeXL = new Node(nodes[2].key, nodes[2].value, nodes[2].tag - 1,
                nodes[2].balance, nodes[2].left, nodes[2].right, dummy);
        final Node nodeXRL = new Node(nodes[4].key, nodes[4].value, nodes[4].tag - 1,
                nodes[4].balance, nodes[4].left, nodes[4].right, dummy);
        final Node nodeXR = new Node(nodes[3].key, nodes[3].value, 0, 0, nodeXRL, nodes[3].right,
                dummy);
        final int tag = (isSentinel(nodes[1]) ? 0 : nodes[1].tag + 1); // root of old subtree is
        // a sentinel
        final Node nodeX = new Node(nodes[1].key, nodes[1].value, tag, -1, nodeXL, nodeXR, dummy);
    ...
    private Operation createPsh12Op(final Node[] nodes, final Operation[] ops) {
        final Node nodeXLL = new Node(nodes[2].key, nodes[2].value, nodes[2].tag - 1,
                nodes[2].balance, nodes[2].left, nodes[2].right, dummy);
        final Node nodeXL = new Node(nodes[1].key, nodes[1].value, 0, r(nodes[4]), nodeXLL,
                nodes[4].left, dummy);
        final Node nodeXR = new Node(nodes[3].key, nodes[3].value, 0, -l(nodes[4]),
                nodes[4].right, nodes[3].right, dummy);
        final int tag = (isSentinel(nodes[1]) ? 0 : nodes[1].tag + 1); // root of old subtree is
        // a sentinel
    ...
    private Operation createPsh13Op(final Node[] nodes, final Operation[] ops) {
        final Node nodeXLL = new Node(nodes[2].key, nodes[2].value, nodes[2].tag - 1,
                nodes[2].balance, nodes[2].left, nodes[2].right, dummy);
        final Node nodeXL = new Node(nodes[1].key, nodes[1].value, 0, r(nodes[4]) - 1, nodeXLL,
                nodes[4].left, dummy);
        final Node nodeXR = new Node(nodes[3].key, nodes[3].value, 0, 1 - l(nodes[4]),
                nodes[4].right, nodes[3].right, dummy);
        final int tag = nodes[1].tag;
        final Node nodeX = new Node(nodes[4].key, nodes[4].value, tag, nodes[4].balance, nodeXL,
                nodeXR, dummy);
        return new Operation(nodes, ops, nodeX);
    }

    private Operation createPullSymOp(final Node[] nodes, final Operation[] ops) {
        final Node nodeXR = new Node(nodes[2].key, nodes[2].value, 0, nodes[2].balance,
                nodes[2].left, nodes[2].right, dummy);
        final int tag = (isSentinel(nodes[1]) ? 0 : nodes[1].tag - 1 + l(nodes[1])); // root of
        // old subtree is a sentinel
        final Node nodeX = new Node(nodes[1].key, nodes[1].value, tag, nodes[1].balance - 1,
                nodes[1].left, nodeXR, dummy);
        return new Operation(nodes, ops, nodeX);
    }

    private Operation createPushSymOp(final Node[] nodes, final Operation[] ops) {
        final Node nodeXR = new Node(nodes[2].key, nodes[2].value, nodes[2].tag - 1,
                nodes[2].balance, nodes[2].left, nodes[2].right, dummy);
        final int tag = (isSentinel(nodes[1]) ? 0 : nodes[1].tag + r(nodes[1])); // root of old
        // subtree is a sentinel
        final Node nodeX = new Node(nodes[1].key, nodes[1].value, tag, nodes[1].balance + 1,
                nodes[1].left, nodeXR, dummy);
        return new Operation(nodes, ops, nodeX);
    ...
    private Operation createPul5SymOp(final Node[] nodes, final Operation[] ops) {
        final Node nodeXL = new Node(nodes[1].key, nodes[1].value, 0, r(nodes[2]) - 1,
                nodes[1].left, nodes[2].left, dummy);
        final int tag = (isSentinel(nodes[1]) ? 0 : nodes[1].tag - 1 + r(nodes[2])); // root of
        // old subtree is a sentinel
        final Node nodeX = new Node(nodes[2].key, nodes[2].value, tag, nodes[2].balance + 1,
                nodeXL, nodes[2].right, dummy);
        return new Operation(nodes, ops, nodeX);
    }

    private Operation createPul6SymOp(final Node[] nodes, final Operation[] ops) {
        final Node nodeXRL = new Node(nodes[3].key, nodes[3].value, nodes[3].tag - 1,
                nodes[3].balance, nodes[3].left, nodes[3].right, dummy);
        final Node nodeXR = new Node(nodes[2].key, nodes[2].value, 0, 0, nodeXRL, nodes[2].right,
                dummy);
        final int tag = nodes[1].tag;
        final Node nodeX = new Node(nodes[1].key, nodes[1].value, tag, -1, nodes[1].left, nodeXR,
                dummy);
        return new Operation(nodes, ops, nodeX);
    }

    private Operation createPul7SymOp(final Node[] nodes, final Operation[] ops) {
        final Node nodeXL = new Node(nodes[1].key, nodes[1].value, 0, r(nodes[3]), nodes[1].left,
                nodes[3].left, dummy);
        final Node nodeXR = new Node(nodes[2].key, nodes[2].value, 0, -l(nodes[3]),
                nodes[3].right, nodes[2].right, dummy);
        final int tag = nodes[1].tag;
        final Node nodeX = new Node(nodes[3].key, nodes[3].value, tag, 0, nodeXL, nodeXR, dummy);
        return new Operation(nodes, ops, nodeX);
    }
    ...
    private Operation createPul8SymOp(final Node[] nodes, final Operation[] ops) {
        final Node nodeXL = new Node(nodes[1].key, nodes[1].value, 0, r(nodes[3]) - 1,
                nodes[1].left, nodes[3].left, dummy);
        final Node nodeXR = new Node(nodes[2].key, nodes[2].value, 0, 1 - l(nodes[3]),
                nodes[3].right, nodes[2].right, dummy);
        final int tag = (isSentinel(nodes[1]) ? 0 : nodes[1].tag - 1); // root of old subtree is
        // a sentinel
        final Node nodeX = new Node(nodes[3].key, nodes[3].value, tag, nodes[3].balance, nodeXL,
    ...
    private Operation createPsh10SymOp(final Node[] nodes, final Operation[] ops) {
        final Node nodeXRR = new Node(nodes[3].key, nodes[3].value, nodes[3].tag - 1,
                nodes[3].balance, nodes[3].left, nodes[3].right, dummy);
        final Node nodeXR = new Node(nodes[1].key, nodes[1].value, 0, 1 - l(nodes[2]),
                nodes[2].right, nodeXRR, dummy);
        final int tag = (isSentinel(nodes[1]) ? 0 : nodes[1].tag + l(nodes[2])); // root of old
        // subtree is a sentinel
        final Node nodeX = new Node(nodes[2].key, nodes[2].value, tag, nodes[2].balance - 1,
                nodes[2].left, nodeXR, dummy);
        return new Operation(nodes, ops, nodeX);
    ...
    private Operation createPsh11SymOp(final Node[] nodes, final Operation[] ops) {
        final Node nodeXLR = new Node(nodes[4].key, nodes[4].value, nodes[4].tag - 1,
                nodes[4].balance, nodes[4].left, nodes[4].right, dummy);
        final Node nodeXL = new Node(nodes[2].key, nodes[2].value, 0, 0, nodes[2].left, nodeXLR,
                dummy);
        final Node nodeXR = new Node(nodes[3].key, nodes[3].value, nodes[3].tag - 1,
    ...
    private Operation createPsh12SymOp(final Node[] nodes, final Operation[] ops) {
        final Node nodeXL = new Node(nodes[2].key, nodes[2].value, 0, r(nodes[4]), nodes[2].left,
                nodes[4].left, dummy);
        final Node nodeXRR = new Node(nodes[3].key, nodes[3].value, nodes[3].tag - 1,
                nodes[3].balance, nodes[3].left, nodes[3].right, dummy);
        final Node nodeXR = new Node(nodes[1].key, nodes[1].value, 0, -l(nodes[4]),
                nodes[4].right, nodeXRR, dummy);
        final int tag = (isSentinel(nodes[1]) ? 0 : nodes[1].tag + 1); // root of old subtree is
        // a sentinel
        final Node nodeX = new Node(nodes[4].key, nodes[4].value, tag, 0, nodeXL, nodeXR, dummy);
        return new Operation(nodes, ops, nodeX);
    ...
    private Operation createPsh13SymOp(final Node[] nodes, final Operation[] ops) {
        final Node nodeXL = new Node(nodes[2].key, nodes[2].value, 0, r(nodes[4]) - 1,
                nodes[2].left, nodes[4].left, dummy);
        final Node nodeXRR = new Node(nodes[3].key, nodes[3].value, nodes[3].tag - 1,
                nodes[3].balance, nodes[3].left, nodes[3].right, dummy);
        final Node nodeXR = new Node(nodes[1].key, nodes[1].value, 0, 1 - l(nodes[4]),
                nodes[4].right, nodeXRR, dummy);
        final int tag = nodes[1].tag;
        final Node nodeX = new Node(nodes[4].key, nodes[4].value, tag, nodes[4].balance, nodeXL,
                nodeXR, dummy);
